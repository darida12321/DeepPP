...
constexpr decltype(auto) print(Layers<ls...> layers) {
  []<std::size_t... I>(Layers<ls...> layers, std::index_sequence<I...>) {
    ((cout << (int)std::get<I>(layers.layers).bias[0] << ", "), ...) << endl;
  }
  (std::forward<Layers<ls...>>(layers), std::index_sequence_for<ls...>{});
}
...
constexpr int get_size(Layers<ls...> layers) {
  ...
  [&size]<std::size_t... I>(Layers<ls...> layers, std::index_sequence<I...>) {
...
int main() {
  Layers<Layer<5>, Layer<2>, Layer<3>> l;
