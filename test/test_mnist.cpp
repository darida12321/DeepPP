...
void printPrediction(VectorXd out) {
    ...
    for (int i = 0; i < 10; i++) {
        std::cout << out(i) << " | ";
    }std::cout << getPrediction(out) << std::endl;
}
...
TEST(MnistTest, ReadData) {
    ...
    // for(int i = 0; i < 3; i++) {
    //     network.trainOne(image.GetImage(i), image.GetLabel(i), 5);
    //     std::cout << getPrediction(image.GetLabel(i)) << " ";
    ...
    // for (int i = 0; i < 10; i++) {
    //     printPrediction(network.forwardProp(image.GetImage(1000+i)));
    //     std::cout << "Actual label: " << getPrediction(image.GetLabel(1000+i)) << std::endl;
    // }
